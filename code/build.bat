...
REM Zi(Z7): produce debug information(.pdb) - things like where does this part in exe correnspond to the code... REM Od : debug mode
...
cl %CommonCompilerFlags% ..\laser_is_dangerous\code\fox.cpp -Fmfox.map -LD /link -incremental:no -opt:ref -PDB:fox_%random%.pdb -EXPORT:GameGetSoundSamples -EXPORT:GameUpdateAndRender
del lock.tmp
cl %CommonCompilerFlags% ..\laser_is_dangerous\code\win32_fox.cpp -Fmwin32_fox.map /link %CommonLinkerFlags%
popd
