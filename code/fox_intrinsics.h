...
inline uint32
AtomCompareExchangeUint32(uint32 volatile *destination, uint32 expectedValue, uint32 newValue)
{
    // IMPORTANT : the return value is value BEFORE it is being changed!
    uint32 result = _InterlockedCompareExchange((long volatile*)destination, expectedValue, newValue);
    return result;
}

#else
// TODO : How can we define these on other compilers?
